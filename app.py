...
with st.sidebar:
    st.markdown(
        '<div class="sidebar-logo">❤️ CardioRisk AI</div>', unsafe_allow_html=True
    )
...
if page == "🔍 Risk Prediction":
    ...
    st.markdown(
        "Enter the patient's clinical characteristics below to calculate their probability of cardiac arrest."
    )

    model, scaler = load_model()
    if model is None:
        st.error(
            "No trained model found. Please upload patient data first to train the model."
        )
        st.stop()
    ...
    with st.form("predict_form"):
        ...
        submitted = st.form_submit_button(
            "🧠 Calculate Risk", use_container_width=True, type="primary"
        )
    ...
    if submitted:
        ...
        high_risk_factors = []
        if age > 60:
            high_risk_factors.append(f"Age ({age} yrs — elevated risk above 60)")
        if chol > 240:
            high_risk_factors.append(f"High cholesterol ({chol} mg/dL)")
        if rbp > 140:
            high_risk_factors.append(f"High resting BP ({rbp} mmHg)")
        if cp == 3:
            high_risk_factors.append(
                "Asymptomatic chest pain (paradoxically high risk)"
            )
        if exang == 1:
            high_risk_factors.append("Exercise-induced angina present")
        if std > 2:
            high_risk_factors.append(f"Significant ST depression ({std})")
        if nv > 0:
            high_risk_factors.append(f"{nv} major vessel(s) affected")
        if thal in [1, 2]:
            high_risk_factors.append("Thalassemia defect detected")
        if mhr < 120:
            high_risk_factors.append(f"Low max heart rate ({mhr} bpm)")
        ...
        if high_risk_factors:
            ...
            st.success(
                "No major individual risk factors flagged from the provided values."
            )
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        "Upload CSV files containing cardiac patient records. The model retrains automatically on each upload."
    )
    ...
    if uploaded:
        try:
            ...
            if st.button(
                "🚀 Save & Train Model", type="primary", use_container_width=True
            ):
                with st.spinner("Training model…"):
                    ...
                    if len(combined) < 20:
                        st.warning(
                            "Need at least 20 rows to train reliably. "
                    ...
                    if combined[TARGET].nunique() < 2:
                        st.error(
                            "Dataset must contain both positive (1) and negative (0) cases."
                        )
                        st.stop()
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(
        f'<div class="metric-box"><div class="metric-val">{meta["auc_test"]:.3f}</div><div class="metric-lbl">AUC — Test Set</div></div>',
        unsafe_allow_html=True,
    )
    c2.markdown(
        f'<div class="metric-box"><div class="metric-val">{meta["auc_cv"]:.3f}</div><div class="metric-lbl">AUC — 5-Fold CV</div></div>',
        unsafe_allow_html=True,
    )
    c3.markdown(
        f'<div class="metric-box"><div class="metric-val">{meta["acc_test"]:.3f}</div><div class="metric-lbl">Accuracy — Test</div></div>',
        unsafe_allow_html=True,
    )
    c4.markdown(
        f'<div class="metric-box"><div class="metric-val">{meta["n_samples"]}</div><div class="metric-lbl">Training Samples</div></div>',
        unsafe_allow_html=True,
    )
    ...
    st.caption(
        f"Last trained: {meta.get('trained_at', '?')[:19]}  |  "
